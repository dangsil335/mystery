...
(function (root) {
  ...
  function pressSuccessLine(s) {
    // 추궁이 통해도 '자백 확정'은 아니다 — 동요만 보일 뿐, 진위 판단은 유저 몫.
    if (s.alibiTruth === 'lie') return `(얼굴이 굳고 시선이 흔들린다) 그, 그건… 제 기억이 분명치 않을 수도 있죠. …하지만 전 결백합니다!`;
    return `(당황하며) 그건… 제 기억이 틀렸을 수도 있죠. 하지만 결백합니다!`;
  }
